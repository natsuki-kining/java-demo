...
### 2.11 伪共享
因为计算机系统中的主内存与CPU之间存在运行速度，所以为了解决这个问题在两者之间添加了一级或者多级告诉缓存存储器，这个cache一般是被集成到cpu内部，也叫cpu cache。  
在cache内部是按行存储的，其中每一行被称为一个cache行。cache行是cache与主内存进行数据交换的单位，cache行的大小一般为2的幂次数字节。   
cpu访问变量时，会先从cache里面找，没有再去主内存里面获取，然后把该变量所在内存区域的一个cache行大小的内存复制到cache中。由于存放到cache行的是内存块而不是单个变量，所以可能会把多个变量存放到一个cache行中。  
